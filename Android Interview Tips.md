...
Java 语言号称一切都是对象，但是原始（基本）数据类型除外。
* int 属于基本数据类型，放在栈中，直接存数值，而 integer 属于复杂数据类型对象，有一个 int 类型的字段存储数据；
...
##### d. Java 使用的不同阶段：编译阶段、运行时，自动装箱 / 自动拆箱是发生在什么阶段？valueOf 会使用到缓存机制，自动装箱的时候，缓存机制起作用吗？

* 自动装箱：自动装箱实际上算是一种语法糖。什么是语法糖？可以简单理解为 Java 平台为我们自动进行了一些转换，保证不同的写法在运行时等价，它们发生在编译阶段，也就是生成的字节码是一致的；javac 替我们自动把装箱转换为 Integer.valueOf()，把拆箱替换为 Integer.intValue()，既然调用的是 Integer.valueOf，自然能够得到缓存的作用。缓存机制并不是只有 Integer 才有，同样存在于其他的一些包装类，比如：Boolean，缓存了 true/false 对应实例，确切说，只会返回两个常量实例 Boolean.TRUE/FALSE。Short，同样是缓存了 -128 到 127 之间的数值。Byte，数值有限，所以全部都被缓存。Character，缓存范围 '\u0000' 到 '\u007F'。

##### e. 自动装箱 / 自动拆箱似乎很酷，在编程实践中，有什么需要注意的吗？

建议避免无意中的装箱、拆箱行为，尤其是在性能敏感的场合，创建 10 万个 Java 对象和 10 万个整数的开销可不是一个数量级的，不管是内存使用还是处理速度，光是对象头的空间占用就已经是数量级的差距了。使用原始数据类型、数组甚至本地代码实现等，在性能极度敏感的场景往往具有比较大的优势，用其替换掉包装类、动态数组（如 ArrayList）等可以作为性能优化的备选项。一些追求极致性能的产品或者类库，会极力避免创建过多对象。

##### f.阅读过 Integer 源码吗？分析下类或某些方法的设计要点。

首先，继续深挖缓存，Integer 的缓存范围虽然默认是 -128 到 127;第二，在分析字符串的设计实现时，提到过字符串是不可变的，保证了基本的信息安全和并发编程中的线程安全。第三，Integer 等包装类，定义了类似 SIZE 或者 BYTES 这样的常量，这反映了什么样的设计考虑呢？如果你使用过其他语言，比如 C、C++，类似整数的位数，其实是不确定的，可能在不同的平台，比如 32 位或者 64 位平台，存在非常大的不同。那么，在 32 位 JDK 或者 64 位 JDK 里，数据位数会有不同吗？因为原始数据类型是不存在差异的，这些明确定义在Java 语言规范里面，不管是 32 位还是 64 位环境，开发者无需担心数据的位数差异。

##### g. 原始类型线程安全

原始数据类型的变量，显然要使用并发相关手段，才能保证线程安全，如果有线程安全的计算需要，建议考虑使用类似 AtomicInteger、AtomicLong 这样的线程安全类。

##### h. Java 原始数据类型和引用类型局限性
...
* 将方法或者类声明为 final，可以明确告诉别人改行为是不可以修改的；
* 使用 final 修饰的参数或者变量，可以避免意外赋值导致的编译错误，有人建议将所有方法参数、本地变量、成员变量声明成 final；
* final 变量产生了某种程度的不可变的效果，可以用于保护只读数据；
* final 不是 immutable：

##### 2）finalize 不推荐使用：

* finalize 的执行和垃圾收集关联在一起，一旦实现了非空的 finalize 方法，就会导致相应对象回收呈现数量级上的变慢；
* 回收资源是因为资源有限，但是垃圾收集时间的不可预测，可能会极大加剧资源的占用；
* 无法保证 finalize 什么时候执行，执行的是否符合预期，使用不当会导致死锁、挂起等；利用 try-with-resources 或者 try-finally 机制来回收资源；如果确实需要处理，可以考虑 cleaner 机制或者其他的替代方法；

##### 3）Cleaner 机制

Cleaner 的实现利用了幻象引用，即常见的 post-mortem 清理机制，利用幻象应用和引用队列，可以保证对象被彻底销毁前做一些类似资源回收的工作，比 finalize 更加轻量、可靠；
...
在 Java 里面，一切都可看作对象，但操纵的标识符实际是指向一个对象的句柄，也可称作引用，甚至是指针

##### 20.2 所以对象都必须创建

创建句柄时，同时会创建一个新对象连接，即我们通常使用的 new 来生成对象类似；

对象的保存位置：

* 寄存器：最快的保存区域，位于处理器内部；数量有限，由编译器分配，无法直接操作；

* 堆栈：驻留于随机访问存储器（RAM）区域，但可通过它的堆栈指针获得处理的直接支持；堆栈向下移，会创建新的内存，若向上移，则会释放那些内存；

* 堆：常规用途的内存池（RAM），保存 Java 对象，与堆栈不同，堆无须知道要从堆里分配多少存储空间，也不必知道存储的数据要在堆里停留多长时间，因此保存数据时具有更大的灵活性，一般通过 new 命令来创建对象都会保存在堆中；

* 静态存储：固定位置存储（RAM），程序运行期间，静态存储的数据将随时等候调用；

* 常数存储： 常数值直接置于程序代码内部，永远不会改变，若有数据需要严格保护，则可以加入只读存储器（ROM）；

* 非 RAM 存储：数据完全独立于程序之外，则程序不运行时仍可存在，并在程序的控制范围之外。

##### 20.3 绝对不要清除对象

* 作用域里定义的一个变量，只有在那个作用域结束之前才可使用
* Java 对象不具备与主类型一样的存在时间，用 new 关键字创建一个 Java 对象的时候，它会超出作用域的范围之外；Java 垃圾收集器会查找用 new 创建的所有对象，并自动释放闲置对象占据的内存，以便能由新对象使用
...
* 位运算符：AND(&)、OR(|)、XOR(^)、NOT(~) => 1/0
* 移位运算符：<<(有符号左移，低位补 0)、>>(有符号右移，正值高位插 0，负值插 1)、>>>(无符号右移，零扩展，高位插 0)
* 三元 if-else 运算符
* 逗号运算符：,(for 循环)
* 字串运算符：+
* 造型运算符：cast
...
### 3. 给出两个整数 a 和 b, 求他们的和, 但不能使用 + 等数学运算符

下面考虑二进制数的情况（6＝0110，11＝1011）：
* 第1步（忽略进位）：0＋1＝1，1＋0＝1，1＋1＝0，0＋1＝1，典型的异或运算;
* 第2步：只有 1＋1 会向前产生进位 1，相对于这一数位的进位值为 10，而 10 ＝ (1 & 1) << 1;
* 第3步：将第 1 步和第 2 步得到的结果相加，其实又是在重复上述 2 步，直到不再产生进位为止;
...
### 4. 设计一个算法，计算出n阶乘中尾部零的个数，如 11! = 39916800，因此应该返回 2，O(logN)的时间复杂度
...
分析上面的数列可知，每 5 个数中会出现一个可以产生结果中 0 的数字。把这些数字抽取出来是：

```xml
...、5、...、10、...、15、...、20、...、25、...
```

这些数字其实是都能满足 5*k 的数字，是 5 的倍数。统计一下他们的数量：n1=N/5。比如如果是 101，则 101 之前应该是 5,10,15,20,...,95,100 共 101/5=20 个数字满足要求。

将 1 中的这些数字化成 5*(1、2、3、4、5、...) 的形式，内部的 1、2、3、4、5、...又满足上面的分析：每 5个数字有一个是 5 的倍数。抽取为：

```xml
...、25、...、50、...、75、...、100、...、125、...
```

而这些数字都是 25 的倍数（5 的 2 次幂的倍数），自然也都满足 5*k 的要求。
这些数字是 25、50、75、100、125、...=5*(5、10、15、20、25、...)=5*5*(1、2、3、4、5、...)，内部的1、2、3、4、5、...又满足上面的分析，因此后续的操作重复上述步骤即可。 

统计一下第二次中满足条件的数字数量：n2=N/5/5，101/25=(101/5)/5=4。 
因为 25、50、75、100、125、...它们都满足相乘后产生至少两个 0，在第一次 5*k 分析中已经统计过一次。
对于 N=101，是 20。因此此处的 5*5*k 只要统计一次 4 即可，不需要根据 25 是 5 的二次幂统计两次。 
后面的 125,250,...等乘积为 1000 的可以为结果贡献 3 个 0 的数字，只要在 5*5*k 的基础上再统计一次 n3=((N/5)/5)/5 即可。 
